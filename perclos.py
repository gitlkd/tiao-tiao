...
def detect_eye_state(frame, counter, total):
    ...
    for face in faces:
        ...
        if ear < EYE_AR_THRESH:
            ...
            # 计算perclos值，即当前窗口内闭眼时间占总时间的比例 # 新增代码
            perclos_value = counter * BLINK_DURATION / window_size

            # 判断perclos值是否超过p80阈值，如果超过则认为出现疲劳驾驶，并在图像上显示警告信息
            if perclos_value > P80_THRESH:
                fatigue_driving = True
                # 在图像上显示中文警告信息，使用putTextZH函数 # 修改代码
                putTextZH(frame, "疲劳驾驶！", (100, 200), "simhei", 1.5, (0, 0, 255), 3)
            else:
        ...
        # 在图像上显示眨眼次数和眼部纵横比
        # cv2.putText(frame, "Blinks: {}".format(total), (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2) # 删除代码
...
# 定义一个函数来在图像上显示中文文字，使用PIL库 # 新增代码
def putTextZH(img, text, point, fontFace, fontSize, color, thickness):
    from PIL import ImageFont, ImageDraw, Image
    pil_img = Image.fromarray(img)
    draw = ImageDraw.Draw(pil_img)
    font = ImageFont.truetype(fontFace + ".ttf", int(fontSize * 10))
...
while True:
    ...
    if current_time > window_start + window_size:
        # 如果超过窗口的结束时间，重置窗口的起始时间和眨眼次数，并将疲劳驾驶标志设为False # 修改代码
